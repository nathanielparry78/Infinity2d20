    ...
    render() {
        ...
        const equipment = this.props.equipment.map((eq, i) => {
            if (eq.indexOf('|') > -1) {
                ...
                const eqList = eqs.map((e, j) => {
                    return (<option key={j} value={e}>{e}</option>)
                });

                const index = this._selectCount++;

                return (
                    <select key={i} onChange={e => this.props.onSelected(eqs[(e.target as HTMLSelectElement).selectedIndex], index) }>
                        {eqList}
                    </select>
                )
            }
